...
var tvii = {
    ...
    posts: {
        getMiiverseParPackProp: function (key) {
            ...
            var decodedParam = atob(param.replace(/\s/g, ''));
        ...
        sendPostToApi: function (
            ...
            // Build OLV crosspost headers so the server can forward to Roseverse
            var olvHeaders = null;
            try {
                if (vino.olv_isEnabled()) {
                    olvHeaders = [
                        "X-Olv-Api-Url:" + vino.olv_getHostName(),
                        "X-Olv-ServiceToken:" + vino.olv_getServiceToken(),
                        "X-Olv-ParamPack:" + vino.olv_getParameterPack(),
                        "X-Olv-UserAgent:" + vino.olv_getUserAgent()
                    ];
                    // Add language_id from ParamPack for OLV crosspost
                    var langId = tvii.posts.getMiiverseParPackProp("language_id");
                    postForm.append("olv_language_id", langId ? langId : "1");
                }
            } catch (e) {
                console.log("[OLV] Could not build crosspost headers:", e);
                olvHeaders = null;
            }

            var url = "/api/v1/socials/postsAlt";

            var xhr = tvii.sendXHR("POST", url, null, null, olvHeaders, postForm)
            xhr.onload = function () {
                onPostSendFinish(xhr.status === 200, xhr.responseText);
            }
            return xhr;
        },
    ...
    sendXHR: function (
        ...
        if (headers) {
            for (var i = 0; i < headers.length; i++) {
                var colonIdx = headers[i].indexOf(":");
                var headerName = headers[i].substring(0, colonIdx).trim();
                var headerValue = headers[i].substring(colonIdx + 1).trim();
    ...
    parseLocalDateTime: function (str) {
        // "2026-01-25T21:00:00" or "2026-01-25 21:00:00"
...
function initVinoSetup() {
    ...
    function checkZipCodeProviders() {
        var code = isJP ? "00000" : (isCA ? caZipCodeInput.val() : usZipCodeInput.val());
        ...
        var endpoint;
        if (isJP) {
            endpoint = "/api/v1/providers/countries/JP/00000";
        } else if (isCA) {
            endpoint = "/api/v1/providers/countries/CA/" + code;
        } else {
            endpoint = "/api/v1/providers/countries/US/" + code;
        }
    ...
    function requestChannelsToFavorite() {
        ...
        var endpoint = "/api/v1/providers/countries/" + (isJP ? "JP" : isCA ? "CA" : "US") + "/" + providerId + "/channels?tz_name=" + providerTz;
    ...
    //Check if its US or Canada or JP
    if (isUS) {
        usZipCodeInput.removeClass("none");
    } else if (isCA) {
        caZipCodeInput.removeClass("none");
    }
    // JP: hide zipcode-related UI, auto-fetch providers when screen 3 is shown
    if (isJP) {
        $(".zipcode-input").addClass("none");
        $(".provider-info").addClass("none");
    } else {
    ...
    // JP: when setup-modal-3 appears, auto-check providers
    if (isJP) {
        var _origChangeSetupModal = changeSetupModal;
        // Monkey-patch: trigger provider load when modal-3 is shown
        var jpProvidersFetched = false;
        $("a[data-show='#setup-modal-3']").on("click", function () {
            if (!jpProvidersFetched) {
                jpProvidersFetched = true;
                setTimeout(function () { checkZipCodeProviders(); }, 100);
            }
        });
    }
...
function initVinoHome() {
    ...
    /**
     * Convert a digit character to its IR keycode.
     *   Standard (TV) digits:  0→20, 1→11, 2→12, …, 9→19
     *   BS  3-digit entry:     0→33, 1→24, 2→25, …, 9→32
     *   CS  3-digit entry:     0→45, 1→36, 2→37, …, 9→44
     *
     * @param {string} ch  Single digit character ("0"–"9")
     * @param {number} base  10 for standard/DT, 23 for BS, 35 for CS
     * @returns {number} IR keycode
     */
    function digitIrCode(ch, base) {
        var d = parseInt(ch, 10);
        if (isNaN(d)) return -1;
        return d === 0 ? base + 10 : base + d;
    }

    /**
     * IR blaster fallback for G-Guide channels when Kodi is unavailable.
     * Sends mode-switch → 49 (channel entry) → 3 encoded digits via
     * ir_sendPack (native Wii U) or sequential ir_send (emulator).
     */
    function sendIrForGGuide(broad, chNum) {
        var modeKey, digitBase;
        switch (broad) {
            case "dt": modeKey = 6;  digitBase = 10; break;
            case "bs": modeKey = 8;  digitBase = 23; break;
            case "cs": modeKey = 9;  digitBase = 35; break;
        ...
        var d1 = digitIrCode(threeDigit[1], digitBase);
        var d2 = digitIrCode(threeDigit[2], digitBase);

        if (typeof vino.ir_sendPack === "function") {
            vino.ir_sendPack(
                modeKey, 550, 49, 300,
                d0, 300, d1, 300, d2, 200, -1, -1
            );
            setTimeout(function () {
    ...
    $(".tune-in").on("click", function (e) {
        ...
        // ── Japanese G-Guide channel tuning ──────────────────
        // G-Guide channel IDs encode the broadcast type:
        //   gguide-dt-{num}  terrestrial (地デジ)
        //   gguide-bs-{num}  BS
        //   gguide-cs-{num}  CS
        //
        // Primary path: Kodi PVR switching via server-side JSON-RPC.
        // Fallback: Wii U GamePad IR blaster (ir_sendPack / ir_send).
        var channelId = $(".program-details").attr("data-channel-id") || "";
        var ggMatch = channelId.match(/^gguide-(dt|bs|cs)-/);

        if (ggMatch) {
            // ─── Try Kodi first ──────────────────────────────
            $.ajax({
                url: "/api/v1/providers/kodi/tune",
                method: "POST",
                ...
                error: function (xhr) {
                    // Kodi unavailable or no match — fall back to IR blaster
                    sendIrForGGuide(ggMatch[1], chNum);
                }
            });
            return;
        }

        // ── US / CA  digit-by-digit tuning ───────────────────
        ...
        function sendNextDigit() {
            ...
            var code = digitIrCode(digit, 10);

            if (code < 0) {
                // "." / "-" / other
                switch (digit) {
                    case ".": case "-": code = 55; break;
                    default:
    ...
    $(".header .menu").on("click", function (e) {
        ...
        // Close button
        menuModal.find(".menu-close").on("click", function (e2) {
            e2.preventDefault();
            vino.soundPlayVolume("SE_WAVE_BTN_CANCEL", 30);
            menuModal.hide();
        });

        // Click on backdrop closes menu
        menuModal.on("click", function (e2) {
            if ($(e2.target).hasClass("menu-modal")) {
                vino.soundPlayVolume("SE_WAVE_BTN_CANCEL", 30);
                menuModal.hide();
            }
        });
        ...
        // Menu item actions
        menuModal.find(".menu-item").on("click", function (e2) {
            e2.preventDefault();
            vino.lyt_startTouchEffect();
    ...
    function setupProgramPage(programObject) {
        ...
        tvii.requestProgramDetails(
            ...
            function (details) {
                ...
                setupProgramActionButtons(det, details);
    ...
    // ========================
    // PROGRAM ACTION BUTTONS (reminder, recommend, view-all-airings)
    // ========================
    function setupProgramActionButtons($det, details) {
        var $reminder = $det.find(".reminder");
        var $recommend = $det.find(".recommend");
        var $viewAll = $det.find(".view-all-airings");
        var listingId = details.program.listingId;
        var channelId = details.channel.id || details.channel.station || "";
        var channelName = details.channel.name || "";
        var programName = details.program.showName || "";
        var startTime = details.program.start || "";

        // Enable the reminder button
        $reminder.removeClass("disabled");
        tvii.setClassHoverToEls($reminder);
        ...
        tvii.sendXHRNoTimeout(
            ...
            function () { /* ignore check errors */ }
        );

        // Reminder click handler
        $reminder.off("click").on("click", function (e) {
            if ($reminder.hasClass("disabled")) return;
            e.preventDefault();

            vino.soundPlayVolume("SE_MOVEPAGE_PLAY", 30);

            var isActive = $reminder.hasClass("active");

            if (isActive) {
                // Remove reminder
                tvii.sendXHRNoTimeout(
                    "DELETE",
                    "/api/v1/act/reminders?listingId=" + encodeURIComponent(listingId),
                    function () {
                        $reminder.removeClass("active");
                        $reminder.find("span").text(tvii.getLoc("vino.home.program.button.set_reminder"));
                    },
                    function () { /* ignore */ }
                );
            } else {
        ...
        // View-all-airings and Recommend: show "not available" for now
        $viewAll.off("click").on("click", function (e) {
            e.preventDefault();
            vino.lyt_startMessage(tvii.getLoc("vino.home.not_available_feature"), null);
        });
        $recommend.off("click").on("click", function (e) {
            e.preventDefault();
            vino.lyt_startMessage(tvii.getLoc("vino.home.not_available_feature"), null);
        });
    }
    ...
    function setMiiUserDetailListener(pid) {
        ...
        tvii.sendXHR("GET", "/api/v1/socials/getUserData/" + pid, function (data) {
            ...
            if (user_data.latest_post_id && vino.olv_isEnabled()) {
    ...
    function buildPostElement(post) {
        ...
        (function (id, authorPid) {
            ...
            jumpPost.on("click", function () {
                ...
                openMiiUserDetailScreen(authorPid);
            });
        })(postId, post.pid);
    ...
    function loadGuideGrid() {
        ...
        var localMs = nowUtcMs + offsetSeconds * 1000;
        var localD = new Date(localMs);
        // Round down to the hour
        localD.setUTCMinutes(0, 0, 0);
        ...
        var guideDuration = 180; // 3 hours in minutes
        var guideLimit = 200; // get all channels
        ...
        tvii.requestProgramGuide(
            ...
            0,
            function (guide) {
                renderGuideGrid(guide, localD, guideDuration);
                vino.loading_setIconAppear(false);
            },
            function () {
                vino.loading_setIconAppear(false);
    ...
    function renderGuideGrid(guide, windowStart, durationMinutes) {
        ...
        var totalWidthPx = 714; // from CSS .guide-container width
        var totalSec = durationMinutes * 60;
        var rowHeight = 68; // from CSS .guide-container>div height
        var slotMinutes = 30;
        var numSlots = durationMinutes / slotMinutes;
        var slotWidthPx = totalWidthPx / numSlots;
        var channelWidth = 140; // left sidebar

        // --- Rebuild guide-view with proper structure ---
        var gv = $(".guide-view");
        gv.empty();
        gv.append('<div class="guide-time-container"></div>');
        gv.append('<div class="guide-channel-container"></div>');
        gv.append('<div class="guide-container l-stick-scroll"></div>');

        // --- Time header ---
        var tc = $(".guide-time-container");
        tc.empty();

        // Label
        var labelA = $("<a>").addClass("label").text(tvii.getLoc("vino.home.header.guide"));
        tc.append(labelA);

        for (var s = 0; s < numSlots; s++) {
            var slotTime = new Date((windowStartSec + s * slotMinutes * 60) * 1000);
        ...
        for (var i = 0; i < data.length; i++) {
            var item = data[i];
            var ch = item.channel;
            var progs = item.programs;

            // Channel entry
            var chA = $("<a>");
            var chSpan1 = $("<span>").text(ch.number || "");
            var chSpan2 = $("<span>").text(ch.name || ch.callsign || "");
            chA.append(chSpan1).append(chSpan2);
            cc.append(chA);
            ...
            if (!progs || progs.length === 0) {
                // Empty row
                var emptyA = $("<a>").css("width", totalWidthPx + "px");
                emptyA.append($("<span>").html("&nbsp;"));
                row.append(emptyA);
            } else {
                var rowStartSec = windowStartSec;

                for (var j = 0; j < progs.length; j++) {
                    var p = progs[j];
                    var pStartSec = tvii.parseLocalDateTime(p.start);
                    var pEndSec = tvii.parseLocalDateTime(p.end);

                    // Clamp to window
                    var visStart = Math.max(pStartSec, windowStartSec);
                    var visEnd = Math.min(pEndSec, windowEndSec);

                    if (visEnd <= visStart) continue;

                    // Gap before this program
                    if (visStart > rowStartSec) {
                        var gapSec = visStart - rowStartSec;
                        var gapPx = (gapSec / totalSec) * totalWidthPx;
                        var gapA = $("<a>").css("width", Math.floor(gapPx) + "px");
                        gapA.append($("<span>").html("&nbsp;"));
                        row.append(gapA);
                    }

                    var durSec = visEnd - visStart;
                    var cellPx = Math.max(30, Math.floor((durSec / totalSec) * totalWidthPx));

                    var cellA = $("<a>")
                        .css("width", cellPx + "px")
                        .attr("data-prlistid", p.listingId || "")
                        .attr("data-chnum", ch.number || "")
                        .attr("data-prgcat", p.showTypeID || "1")
                        .attr("navi_target", "")
                        .attr("tabindex", "0");

                    var nameSpan = $("<span>").text(p.showName || "");
                    var timeSpan = $("<span>");

                    var pStartD = new Date(pStartSec * 1000);
                    var pEndD = new Date(pEndSec * 1000);
                    timeSpan.text(
                        pad(pStartD.getUTCHours()) + ":" + pad(pStartD.getUTCMinutes()) +
                        " - " +
                        pad(pEndD.getUTCHours()) + ":" + pad(pEndD.getUTCMinutes())
                    );

                    cellA.append(nameSpan).append(timeSpan);

                    // Click handler to open program details
                    (function (prog, channel) {
                        cellA.on("click", function () {
                            vino.soundPlayVolume("SE_APPEAR_DETAIL", 30);
                            openGuideProgramDetail(prog, channel);
                        });
                    })(p, ch);
                ...
                if (rowStartSec < windowEndSec) {
                    var trailGapSec = windowEndSec - rowStartSec;
                    var trailGapPx = (trailGapSec / totalSec) * totalWidthPx;
                    var trailA = $("<a>").css("width", Math.floor(trailGapPx) + "px");
                    trailA.append($("<span>").html("&nbsp;"));
                    row.append(trailA);
                }
            }
        ...
        gc.off("scroll.guide").on("scroll.guide", function () {
            cc.scrollTop(gc.scrollTop());
        });
    ...
    function openGuideProgramDetail(prog, channel) {
        // Load program details page similar to how live tab does it
        var detPage = $(".program-fulldetails-page");
        var detTemplate = $(".program-fulldetails-page-template").html();
        if (detTemplate) {
            detPage.html(detTemplate);
            tvii.templates.setUpLocHTML();
        }

        $(".footer .bottom").removeClass("guideopt").addClass("prfuldet");
        tvii.pushStateWithQuery("scene", "guidedetail", false);

        // Fill in program info
        var det = detPage;

        // Set channel info
        det.find(".chname").text(channel.name || "");
        det.find(".chnumber").text(channel.number || "");
        if (channel.logo) {
            det.find(".chlogo").attr("src", "/images/cdn/tvp/" + channel.logo).show();
        }

        // Set description
        if (prog.description) {
            det.find(".program-description p").first().text(prog.description);
            det.find(".parent-description p").text(prog.description);
        }
        if (prog.showName) {
            det.find(".no-episode").text(prog.showName);
        }

        // Show times
        var pStartSec = tvii.parseLocalDateTime(prog.start);
        var pEndSec = tvii.parseLocalDateTime(prog.end);
        var dateStr = "";
        if (pStartSec && pEndSec) {
            dateStr = formatAMPMWithDate(pStartSec, pEndSec);
        }
    ...
    function initRecommendedTab() {
        ...
        // Change the tip text to recommended context
        var tipSpan = document.querySelector(".program-list .content .tips span:nth-of-type(2)");
        if (tipSpan) tipSpan.innerHTML = tvii.getLoc("vino.home.tips.tip1");
        ...
        tvii.requestProgramGuide(
            ...
            0,
            function (guide) {
                // Flatten all currently-airing + upcoming programs
                var nowTimestamp = ((Date.now() / 1000) | 0) + tvii.getUtcOffset();
                var recommended = [];

                for (var i = 0; i < guide.data.length; i++) {
                    var item = guide.data[i];
                    var ch = item.channel;
                    var progs = item.programs;

                    for (var j = 0; j < progs.length; j++) {
                        var p = progs[j];
                        var pStart = tvii.parseLocalDateTime(p.start);
                        var pEnd = tvii.parseLocalDateTime(p.end);

                        // Include programs that are airing now or start within 3 hours
                ...
                var recData = {
                    ...
                    data: recommended.slice(0, maxRecommended)
                };

                total = recData.total;
                setProgramDivAttribute(recData);
                updateTabListProgram();
                setUpTitleScrollbar(programPreviewUpdate, programConfirmSel);
                window.setListenerToProgram();
                setupProgramTimer();
